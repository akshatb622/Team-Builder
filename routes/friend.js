...
router.get("/chat/addfriend/:ui",auth,async (req,res)=>{
    await User.findOne({email:req.user.email},async (err,user)=>{
       if(err) Error(err);
       await User.findOne({_id:req.params.ui},async (err,friend)=>{
            if(err) Error(err);
            let flag = 1;
