...
router.post('/dashboard/editprofile/',auth,async (req,res)=>{
...
router.post('/dashboard/editprofile/addskill/',auth,async (req,res)=>{
    await User.findOne({_id: req.user._id},async(err,user)=>{
        if(err) Error(err);
...
router.post('/dashboard/editprofile/del',auth,async (req,res)=>{
    await User.findOne({_id: req.user._id},async(err,user)=>{
        if(err) Error(err);
...
router.post('/hackathons/insert/:hackathonid/:userid',auth, async (req,res)=>{
    await Hackathon.findOne({_id: req.params.hackathonid},async (err, hackathon)=>{
...
router.delete('/hackathons/insert/:hackathonid/:userid',auth, async (req,res)=>{
    await Hackathon.findOne({_id: req.params.hackathonid})
    .exec(async (err, hackathon)=>{
        if(err) Error("Could not process fetch request "+ err);
        else if(!hackathon) console.log("Hackathon id invalid");
        else {
