...
app.get("/",async(req,res)=>{
    ...
    if(req.session.token == null){
      ...
      await User.findOne({email:req.user.email}).then((err,user)=>{
          if(err) Error(err);
          if(user){
...
io.on('connection',(socket)=>{
  ...
  socket.on('chatMessage',async ({msg,friend_id,user_id})=>{

    await User.findOne({_id:user_id}).then(async(err,user)=>{
      await User.findOne({_id:friend_id}).then(async (err,friend)=>{
